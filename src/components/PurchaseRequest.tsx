  ...
  return (
    <div className="h-full flex flex-col space-y-6 bg-slate-50/50 p-6 overflow-y-auto no-scrollbar">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-black uppercase tracking-tight">Purchase Requests</h1>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-slate-400">PR Details</th>
                <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-slate-400">Project / BOM</th>
                <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-slate-400 text-center">Required Date</th>
              ...
              {records.map((record) => (
                <tr key={record._id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-8 py-6">
                    <p className="text-xs font-black text-black">{record.prNumber}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">{new Date(record.requestDate).toLocaleDateString()}</p>
                  </td>
                  <td className="px-8 py-6">
                    <p className="text-xs font-black text-black">{record.project}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">{record.bom || 'Manual'}</p>
                  </td>
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button 
                        ...
                        className="p-2 bg-slate-50 text-slate-400 hover:text-black hover:bg-slate-100 rounded-xl transition-all"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button 
                        onClick={() => handleDelete(record._id)}
                        className="p-2 bg-slate-50 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-5xl rounded-[40px] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-black text-black uppercase tracking-tight">{editingId ? 'Edit PR' : 'Purchase Request'}</h2>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">Material Requisition Form</p>
              </div>
            ...
            <div className="p-8 overflow-y-auto space-y-8 no-scrollbar flex-1">
              <div className="grid grid-cols-3 gap-6">
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">BOM Reference</p>
                  <select 
                    value={formData.bom}
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Requested By</p>
                  <select 
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Department</p>
                  <select 
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Priority</p>
                  <select 
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Required Date</p>
                  <input type="date" value={formData.requiredDate} onChange={(e) => setFormData({...formData, requiredDate: e.target.value})} className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-black/5 transition-all" />
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-black uppercase tracking-widest text-black">Request Items</h3>
                  <button onClick={handleAddItem} className="bg-black text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
                    <Plus size={14} /> Add Item
                  </button>
                </div>
                <div className="bg-slate-50 rounded-[32px] p-4 overflow-x-auto">
                  <table className="w-full text-left border-separate border-spacing-y-2">
                    <thead>
                      <tr>
                        ...
                        <th className="px-4 py-2 text-[8px] font-black uppercase text-slate-400 text-center">Avail</th>
                        <th className="px-4 py-2 text-[8px] font-black uppercase text-slate-400 text-center">Shortage</th>
                        <th className="px-4 py-2 text-[8px] font-black uppercase text-slate-400 text-right">Est. Cost</th>
                        <th className="px-4 py-2 text-[8px] font-black uppercase text-slate-400 text-center">Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      {formData.items.map((item, index) => (
                        <tr key={index} className="bg-white rounded-2xl shadow-sm">
                          ...
                          <td className="px-4 py-3 text-center">
                            <input type="number" value={item.requestedQuantity} onChange={(e) => handleItemChange(index, 'requestedQuantity', Number(e.target.value))} className="w-16 bg-slate-50 rounded-lg px-2 py-1 text-[10px] font-bold text-center focus:outline-none" />
                          </td>
                          <td className="px-4 py-3 text-center text-[10px] font-bold text-slate-400">{item.availableQuantity}</td>
                          <td className="px-4 py-3 text-center">
                            <span className={`text-[10px] font-black ${item.shortageQuantity > 0 ? 'text-red-500' : 'text-emerald-500'}`}>{item.shortageQuantity}</span>
                          </td>
                          <td className="px-4 py-3 text-right">
                            <input type="number" value={item.estimatedCost} onChange={(e) => handleItemChange(index, 'estimatedCost', Number(e.target.value))} className="w-20 bg-slate-50 rounded-lg px-2 py-1 text-[10px] font-bold text-right focus:outline-none" />
                          </td>
              ...
              <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-3 text-amber-500 bg-amber-50 px-4 py-2 rounded-xl">
                  ...
                  <p className="text-[9px] font-black uppercase tracking-widest">Inventory mapping is active</p>
                </div>
                <div className="flex gap-4">
                  ...
                  <button onClick={handleSave} className="bg-black text-white px-12 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-black/20 hover:scale-[1.02] active:scale-[0.98] transition-all">{editingId ? 'Update PR' : 'Submit PR'}</button>
                </div>
              </div>
            </div>
          </div>
